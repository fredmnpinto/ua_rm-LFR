...
static State s_stateVerifyTarget = {stateVerifyTarget_onEnter, stateVerifyTarget_onTick,
                                    NULL, "VERIFY_TARGET"};
static State s_stateAtTarget = {NULL, stateAtTarget_onTick, NULL, "AT_TARGET"};
static State s_stateApproachCenter = {stateApproachCenter_onEnter, stateApproachCenter_onTick,
                                      NULL, "APPROACH_CENTER"};
static State s_stateChooseNext = {NULL, stateChooseNext_onTick, NULL, "CHOOSE_NEXT"};
static State s_stateTurnToEdge = {stateTurnToEdge_onEnter, stateTurnToEdge_onTick,
...
static State s_stateReturnFollow = {NULL, stateReturnFollow_onTick, NULL, "RETURN_FOLLOW"};
static State s_stateReturnAtNode = {NULL, stateReturnAtNode_onTick, NULL, "RETURN_AT_NODE"};
...
static const char *nodeTypeToString(NodeType type) {
    switch (type) {
        case NODE_TYPE_INTERSECTION: return "INTERSECTION";
        case NODE_TYPE_LEFT_CORNER:  return "LEFT_CORNER";
        case NODE_TYPE_RIGHT_CORNER: return "RIGHT_CORNER";
        case NODE_TYPE_DEADEND:      return "DEADEND";
...
static void stateFollowLine_onTick(void) {
    ...
    bool leftPath = ((s_ground & SENSOR_LEFT) == SENSOR_LEFT) && (s_ground != SENSOR_ALL);
...
static void stateApproachCenter_onTick(void) {
    ...
    getRobotPos(&x, &y, &h);
    dist = hypot(x - s_approachStartX, y - s_approachStartY);
    ...
    if (dist < INTERSECTION_CENTER_DIST) {
        ...
        if (existingId >= 0) {
            ...
            navGraph_setCurrentNode(currentNode);
            isNewNode = true;
            log_nodeAdd(s_tick, currentNode, x, y, nodeTypeToString(s_detectedNodeType));
        }
        ...
        if (s_prevNodeId != currentNode) {
            ...
            if (prevNode) {
                ...
                bool edgeCreated = navGraph_addEdge(s_prevNodeId, currentNode, (float)edgeDist, &edgeId);
                if (edgeCreated) {
                    log_edgeAdd(s_tick, edgeId, s_prevNodeId, currentNode, (float)edgeDist);
...
static void stateChooseNext_onTick(void) {
    ...
    if (currentNode == 0 && navGraph_allEdgesExplored() && navGraph_getTargetNode() != 0xFF) {
    ...
    switch (node->type) {
        case NODE_TYPE_INTERSECTION:
            hasLeft = true; hasStraight = true; hasRight = true; break;
        case NODE_TYPE_LEFT_CORNER:
            hasLeft = true; hasStraight = true; break;
        case NODE_TYPE_RIGHT_CORNER:
            hasRight = true; hasStraight = true; break;
        case NODE_TYPE_DEADEND:
            break;
    }

    /* Check ALL edges of current node for unexplored status regardless of node type.
       Node type is only used as a hint for which new directions are available. */
    double currentHeading = s_robotH;
    double tolerance = 0.6; /* ~35 degrees */
    char edgeId;

    /* Check left */
    edgeId = navGraph_getEdgeInDirection(currentNode, currentHeading, PI / 2.0, tolerance);
    if (edgeId < 0) {
        if (hasLeft) {
    ...
    /* Check straight */
    edgeId = navGraph_getEdgeInDirection(currentNode, currentHeading, 0.0, tolerance);
    if (edgeId < 0) {
        if (hasStraight) {
...
static void stateReturnTurn_onEnter(void) {
    ...
    double deltaAngle = normalizeAngle(atan2(nn->pose.y - cn->pose.y, nn->pose.x - cn->pose.x) - s_robotH);
...
static void startReturnNavigation(void) {
    unsigned char targetNode = navGraph_getTargetNode();
    if (targetNode == 0xFF || !navGraph_dijkstra(0, targetNode, s_returnPath, &s_returnPathLen)) {

...
bool navGraph_addEdge(unsigned char nodeA, unsigned char nodeB, float distance, unsigned char *outEdgeId) {
    char existing;
    unsigned char id;
    if (s_edgeCount >= MAX_EDGES || nodeA >= s_nodeCount || nodeB >= s_nodeCount) {
        return false;
    }
    if (nodeA == nodeB) {
        return false;
...
char navGraph_getEdgeInDirection(unsigned char nodeId, double currentHeading, double desiredDelta, double tolerance) {
    ...
    for (i = 0; i < node->edgeCount; i++) {
        ...
        unsigned char otherNodeId = (edge->nodeA == nodeId) ? edge->nodeB : edge->nodeA;
...
bool navGraph_dijkstra(unsigned char fromNode, unsigned char toNode, unsigned char *outPath, unsigned char *outPathLen) {
    ...
    if (fromNode >= s_nodeCount || toNode >= s_nodeCount || outPath == 0 || outPathLen == 0) {
        return false;
    }
    if (fromNode == toNode) {
        outPath[0] = fromNode;

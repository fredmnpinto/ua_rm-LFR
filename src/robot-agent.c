/**
 * \file robot-agent.c
 * \brief TP2 Path Finder Robot — Left-turn exploration with parent-pointer return
 *
 * Phase 1: Follow black lines, turn left at intersections (explores acyclic tree)
 * Phase 2: Detect wide black line target area, stop, record pose
 * Phase 3: Return to start via shortest path (parent-pointer DFS stack)
 *
...
#define TARGET_TICK_THRESHOLD 10
#define TARGET_DIST_THRESHOLD 0.08
#define BASE_SPEED            50
#define TURN_SPEED            40
#define RECOVERY_SPEED        30
...
#define LOG_TICK() do { g_missionTick++; g_logThisTick = false; } while (0)
...
static const char* stateName(RobotState s)
{
    switch (s) {
        ...
        case STATE_DETECT_INTERSECTION: return "DETECT_INTERSECTION";
        case STATE_TURN_LEFT:           return "TURN_LEFT";
        case STATE_VERIFY_TARGET:       return "VERIFY_TARGET";
        case STATE_AT_TARGET:           return "AT_TARGET";
        case STATE_RETURN_TURN:         return "RETURN_TURN";
        case STATE_RETURN_FOLLOW:       return "RETURN_FOLLOW";
...
static void formatGroundHex(unsigned int ground, char *out)
{
    ...
    out[0] = 'g'; out[1] = 'r'; out[2] = 'o'; out[3] = 'u';
    out[4] = 'n'; out[5] = 'd'; out[6] = '='; out[7] = '0'; out[8] = 'x';
...
{
    if (g_logThisTick) return;
    printf("[T %u] %s -> %s%s%s\n",
           g_missionTick, stateName(g_state), stateName(to),
...
static void logMissionDone(double tx, double ty, double th, int depth, int ticks)
{
    if (g_logThisTick) return;
...
#define LOG_TICK() ((void)0)

#define logTransition(to, detail) ((void)0)
#define logTarget(x, y, h) ((void)0)
#define logPush(depth, x, y, h) ((void)0)
#define logPop(depth) ((void)0)
#define logTurnStart(targetAngle) ((void)0)
#define logTurnDone() ((void)0)
#define logLostTimeout() ((void)0)
#define logMissionDone(tx, ty, th, depth, ticks) ((void)0)
 ...
 */
static void lineFollowTick(unsigned int ground, bool *pIntersection, bool *pLost)
{
    *pIntersection = false;
    *pLost = false;
    ...
    if (popcount >= 3 || ground == SENSOR_ALL) {
...
static void startReturnToParent(void)
{
    ...
    double deltaAngle = normalizeAngle(
        atan2(g_returnTargetY - y, g_returnTargetX - x) - h);
        ...
        switch (g_state) {
            ...
            case STATE_RETURN_FOLLOW:
                ...
                if (dist < ORIGIN_TOLERANCE) {
                    ...
                    if (isStackEmpty()) {
                        /* Back at start */
                        logMissionDone(g_targetX, g_targetY, g_targetH, g_stack_top + 1, g_missionTick);
